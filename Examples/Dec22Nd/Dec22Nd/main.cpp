	...
	__in_ecount(argc) LPWSTR *wargv)
{
	...
	pbCipherText = (PBYTE)HeapAlloc(GetProcessHeap(), 0, 161);
		...
		BCRYPT_BLOCK_PADDING)))
	{
		...
		unsigned char *prtToValue = pbCipherText;
		std::string* base64String;
		int OutCome[192]; //= ( char*)calloc(cbCipherText, sizeof(char));
		 int *prtToOutCOme = OutCome;
		for (int k = 0; k < cbCipherText; k++){
			OutCome[k] = (char)prtToValue++;
			//*prtToOutCOme++ = *prtToValue++;
			//sprintf(prtToOutCOme++,"%c", (unsigned char)prtToValue++);
		}
		char buf[192];

		memcpy(buf, pbCipherText,cbCipherText);
