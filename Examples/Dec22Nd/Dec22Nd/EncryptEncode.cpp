...
std::string Decoder::encode(const std::vector<BYTE>& buf)
{
	return encode(&buf[0], (unsigned int)buf.size());
}

std::string Decoder::encode(const BYTE* buf, unsigned int bufLen)
{
...
std::vector<BYTE> Decoder::decode(std::string encoded_string)
{
